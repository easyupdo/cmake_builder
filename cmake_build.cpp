...
QString cmake_build::target_link_libraries(QString exec_name,QString lib_path_name)//argv1 depend argv2 (lib)
{

    return SetDoubleParameter("target_link_libraries",exec_name,lib_path_name,CMAKE_STRING);
}


 QString cmake_build::FindLibrary(QString argv,QString lib_name,QString lib_path)//lib_path 可以是多个路径 中间用空格分开
 {
        return SetTribleParameter("FIND_LIBRARY",argv,lib_name,lib_path);
 }
...
QString cmake_build::SetTribleParameter(QString options,QString Parameter1,QString Parameter2,cmake_type type1,QString lib_type)//add_library的时候使用
{
    if(type1 == 0)//string
...
QString cmake_build::SetTribleParameter(QString options,QString Parameter1,QString Parameter2,QString Parameter3 )//find_x的时候使用
{
        return options +QString::fromLocal8Bit("(")+Parameter1 + " "  + Parameter2 + " " + Parameter3 + QString::fromLocal8Bit(")");
}
